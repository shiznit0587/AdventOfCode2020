...
let day17 =
    ...
    let state =
        Array4D.zeroCreate<bool> (Array2D.length1 input + 14) (Array2D.length2 input + 14) 15 15

    // ~ Array2D.blit input 0 0 state.[*, *, 7] 7 7 (Array2D.length1 input) (Array2D.length2 input) ~
    // Have to manually perform the 2d blit, since state is `Array3D` and any slice of it passed to `Array2D.blit` is a copy.
    for y = 0 to Array2D.length2 input - 1 do
        for x = 0 to Array2D.length1 input - 1 do
            state.[x + 7, y + 7, 7, 7] <- input.[x, y]
    ...
    let evalCubeState (state: bool [,,,]) x y z w =
        let activeCount =
            countActiveCubes state.[x - 1..x + 1, y - 1..y + 1, z - 1..z + 1, w - 1..w + 1]

        match state.[x, y, z, w], activeCount with
        // Match with 3 and 4 instead of 2 and 3 because the current cube was also counted.
        | true, 3
        | true, 4 -> true
        | true, _ -> false
    ...
    let mapCubeState state x y z w _ =
        // Skip the outer-most layer. It's only in the array to avoid index-out-of-bounds issues with the slice in `evalCubeState`
        match x, y, z, w with
        | x, _, _, _ when x = 0 || x = (Array4D.length1 state - 1) -> false
        | _, y, _, _ when y = 0 || y = (Array4D.length2 state - 1) -> false
        | _, _, z, _ when z = 0 || z = (Array4D.length3 state - 1) -> false
        | _, _, _, w when w = 0 || w = (Array4D.length4 state - 1) -> false
        | _ -> evalCubeState state x y z w

    let state =
        [ 1 .. 6 ]
        |> Seq.fold (fun s _ -> array4dmapi (mapCubeState s) s) state
